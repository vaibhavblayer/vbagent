...
@pytest.mark.parametrize("subject", VALID_SUBJECTS)
def test_problem_only_subjective_uses_plain_nested_enumerate(subject):
    import importlib
    ...
    assert r"\arabic" in prompt
    assert "`tasks` is only for selectable answer choices" in prompt


@pytest.mark.parametrize("subject", VALID_SUBJECTS)
def test_full_subjective_prompt_uses_plain_subpart_contract(subject):
