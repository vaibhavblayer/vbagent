"""Stereochemistry-focused organic diagram agent using chemfig.

Specializes in 3D representation, chiral centers, and stereochemical notation.
"""
...
from vbagent.prompts.diagram.chemistry import organic_stereo
from vbagent.utils.latex import clean_latex_output


def generate_stereochemistry(
    ...
    """Generate chemfig code for a structure with stereochemistry.
    ...
    Returns:
        chemfig code with wedge-dash notation
    """
    if not image_path and not description:
        raise ValueError("Either image_path or description must be provided")
    
    # Format context info
    context_info = organic_stereo.format_context_info(chemistry_context or {})
    
    # Create agent
    agent = create_agent(
        name="StereochemistrySpecialist",
        instructions=organic_stereo.SYSTEM_PROMPT,
        agent_type="tikz",
    )
    ...
    if image_path:
        template = (
            organic_stereo.USER_TEMPLATE_MCQ_OPTIONS if mcq_options
            else organic_stereo.USER_TEMPLATE
        )
        ...
        # Use replace instead of format to avoid issues with curly braces in context_info
        user_prompt = organic_stereo.USER_TEMPLATE.replace("{context_info}", context_info)
        if description:
