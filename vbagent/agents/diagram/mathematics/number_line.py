...
Generates number lines, inequality solutions, intervals,
...
def create_number_line_agent(use_context: bool = True) -> "Agent":
    """Create a number line generation agent.
    ...
    if use_context:
        context = get_number_line_context_for_classification()
        if context:
            prompt = prompt + "\n\n" + context
...
def generate_number_line(
    ...
    """Generate TikZ code for a number line or inequality.
    
    Args:
        image_path: Path to number line image (optional)
        description: Text description of number line (optional)
        use_context: Whether to include reference context
    ...
    agent = create_number_line_agent(use_context)
...
def validate_number_line_output(tikz_code: str) -> tuple[bool, str]:
    """Validate TikZ code for number lines.
    ...
    # Check for number line elements
    has_line = "\\draw" in tikz_code
    if not has_line:
        return False, "Missing number line (\\draw command)"
...
def get_number_line_context_for_classification() -> str:
    """Get reference context for number line generation.
    ...
    results = store.search(
        query="number line inequality interval solution set tikz",
    ...
    context = "\n\n## Reference Examples\n\n"
    context += "Here are some similar number lines from our reference library:\n\n"
