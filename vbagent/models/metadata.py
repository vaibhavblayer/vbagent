...
class StageMetadata(BaseModel):
    ...
    agent_name: Optional[str] = Field(
        default=None, description="Agent that processed this stage")
    model_name: Optional[str] = Field(
        default=None, description="LLM model used")
    error_message: Optional[str] = Field(default=None)
    content_hash: Optional[str] = Field(
        default=None, description="SHA256 hash of output content")
    cache_path: Optional[str] = Field(
        default=None, description="Path to cached content")
...
class PipelineMetadata(BaseModel):
    ...
    solution: Optional[StageMetadata] = Field(
        default=None, description="Solution generation stage")
    tikz: Optional[StageMetadata] = Field(default=None)
    options: Optional[StageMetadata] = Field(
        default=None, description="MCQ option diagrams stage")
    ideas: Optional[StageMetadata] = Field(default=None)
    idea_latex: Optional[StageMetadata] = Field(
        default=None, description="Idea LaTeX block cache")
    animation_assessment: Optional[StageMetadata] = Field(
        default=None, description="Animation assessment cache")
    animation_code: Optional[StageMetadata] = Field(
        default=None, description="Animation Manim code cache")
    solution_script: Optional[StageMetadata] = Field(
        default=None, description="Solution video narration script cache")
    solution_video_code: Optional[StageMetadata] = Field(
        default=None, description="Solution video Manim code cache")
    solution_voice: Optional[StageMetadata] = Field(
        default=None, description="Solution video voice audio cache")
    alternates: Dict[int, StageMetadata] = Field(
        default_factory=dict, description="Alternate solutions by index")
    variants: Dict[str, StageMetadata] = Field(
        default_factory=dict, description="Variants by type")
    ...
    def calculate_summary(self):
        ...
        stages = [
            ...
            self.animation_code,
            self.solution_script,
            self.solution_video_code,
            self.solution_voice,
        ]
...
class CacheIndex(BaseModel):
    ...
    entries: Dict[str, "CacheEntry"] = Field(
        default_factory=dict, description="Hash -> CacheEntry mapping")
...
class CacheEntry(BaseModel):
    ...
    used_by: list[str] = Field(
        default_factory=list, description="Problem IDs using this content")
