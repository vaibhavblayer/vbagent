...
% Connecting lines
\\draw[dashed, thin] (1,0) -- (2.5,0.5);
\\draw[dashed, thin] (1,0) -- (2.5,1.5);
\\draw[dashed, thin] (1,2.2) -- (2.5,2.6);
\\draw[dashed, thin] (1,2.2) -- (2.5,3.5);
...
% Transition
\\draw[->,thick] (2,3.5) -- (2,1) node[midway,right] {$\\Delta E$};
\\end{tikzpicture}
```
...
\\draw[thick] (3,1.1) -- (4,1.1) node[right] at (4,0.8) {sp$^3$};
\\end{tikzpicture}
```
...
**p Orbital (dumbbell) — use fill=white with outlines:**
```latex
\\begin{tikzpicture}
% Upper lobe (filled)
\\draw[thick, fill=black!15] (0,0.8) circle (0.5);
% Lower lobe (empty)
\\draw[thick, fill=white] (0,-0.8) circle (0.5);
% Node line
...
4. **Energy Ordering**: Follow Aufbau principle
5. **Labels**: Clearly label all orbitals and energy levels
6. **No Colors**: Use fill=white, fill=black!15, or patterns to distinguish lobes
7. **Bonding/Antibonding**: Use $\\sigma$, $\\sigma^*$, $\\pi$, $\\pi^*$ notation
8. **Energy Scale**: Show relative energy differences accurately
9. **Dashed lines**: Use for connecting lines in MO diagrams
...
1. NO colors — use fill=white, fill=black!15, patterns for distinction
2. Use TikZ for orbital diagrams (not chemfig)
3. Follow quantum mechanical principles
4. Show correct electron configurations
5. Use proper orbital notation
6. Indicate spin states with arrows
7. Show energy ordering correctly
8. Label all orbitals clearly
