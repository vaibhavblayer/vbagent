...
3. **Concludes clearly**: State the final answer with the actual lowercase option letter, for example: "Therefore, the correct option is (c)."
...
The final line must use the actual lowercase option letter and plain formatting. When it is inside `align*`, write it as `\intertext{Therefore, the correct option is (c).}`. When it is outside `align*`, write `Therefore, the correct option is (c).` as plain text.
