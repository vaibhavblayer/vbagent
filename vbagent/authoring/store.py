...
class AuthoringStore:
        ...
        with self._transaction() as conn:
            ...
            conn.execute(
                ...
                INSERT INTO authoring_runs (
                    ...
                    max_attempts, concurrency, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    plan.plan_id,
    ...
    def acquire_run_lease(self, run_id: str, owner: str, lease_seconds: int = 300) -> bool:
        ...
        with self._transaction() as conn:
            ...
            conn.execute(
                ...
                WHERE run_id = ?
                """,
                (RunStatus.RUNNING.value, owner, expires, _timestamp(now), run_id),
            )
            return True
    ...
    def _validate_candidate_record(
        ...
        *,
        ...
        if claimed.spec.diagram_policy.value == "required":
    ...
    def abandon_item(self, claimed: ClaimedItem, reason: str) -> bool:
        ...
        with self._transaction() as conn:
            ...
            conn.execute(
                ...
                UPDATE authoring_items
                SET status = ?, next_attempt_at = ?, lease_owner = NULL,
                    lease_expires_at = NULL, error_stage = 'infrastructure',
    ...
    def request_cancel(self, run_id: str, reason: str = "user requested cancellation") -> None:
        ...
        with self._transaction() as conn:
            conn.execute(
                """
                UPDATE authoring_runs SET status = ?, cancel_reason = ?, updated_at = ?
            ...
            conn.execute(
                """
                UPDATE authoring_items SET status = ?, completed_at = ?, updated_at = ?
                WHERE run_id = ? AND status = ?
                """,
                (ItemStatus.CANCELLED.value, now, now, run_id, ItemStatus.PENDING.value),
            )
    ...
    def resume_run(self, run_id: str) -> int:
        ...
        with self._transaction() as conn:
            ...
            reset = conn.execute(
                ...
                WHERE run_id = ? AND status = ? AND attempts < max_attempts
                """,
                (ItemStatus.PENDING.value, now, now, run_id, ItemStatus.CANCELLED.value),
            ).rowcount
            conn.execute(
                """
                UPDATE authoring_runs
    ...
    def review_item(self, run_id: str, spec_id: str, *, approve: bool, reason: str) -> None:
        ...
        try:
            with self._transaction() as conn:
                ...
                _atomic_write(review_path, artifact_content.rstrip() + "\n")
    ...
    def refresh_run_status(self, run_id: str) -> RunStatus:
        with self._transaction() as conn:
            ...
            completed_at = _timestamp() if new_status in {RunStatus.COMPLETED, RunStatus.CANCELLED} else None
            conn.execute(
                """
                UPDATE authoring_runs SET status = ?, completed_at = ?, updated_at = ?
                WHERE run_id = ?
                """,
                (new_status.value, completed_at, _timestamp(), run_id),
            )
            return new_status
    ...
    def usage_summary(self, run_id: str) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            usage = candidate.get("provenance", {}).get("usage", {}).get("totals", {})
            for key in totals:
